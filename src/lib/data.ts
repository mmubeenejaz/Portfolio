export const profile = {
  name: "Mubeen Ejaz",
  role: "Business Analyst · Product Owner · AI Engineer",
  tagline:
    "I turn ambiguous business problems into shipped software, and increasingly, I build the solution myself.",
  email: "m.mubeenejaz@gmail.com",
  phone: "+92 342 4500018",
  linkedin: "https://www.linkedin.com/in/mubeenejaz",
  github: "https://github.com/mmubeenejaz",
  summary:
    "For 4+ years at Reporteq, I've turned ambiguous business problems into shipped software for 50+ clients across healthcare, real estate, automotive, and AI-driven platforms, running discovery workshops, authoring BRDs and BPMN process models, and owning delivery from requirements through UAT, while maintaining a 95% client satisfaction rate. I think the next generation of technical business analysts and product owners won't stop at defining what to build; they'll be able to build a working version of it themselves. That's the space I work in: requirements and stakeholder management on one side, hands-on AI application development on the other.",
};

export const skillGroups = [
  {
    title: "AI & Development",
    skills: [
      "Claude Code",
      "Cursor",
      "ChatGPT",
      "Microsoft 365 Copilot",
      "AI Agents",
      "LLMs",
      "Generative AI",
      "Prompt-Based Workflows",
      "Workflow Automation",
      "Next.js",
      "Prisma",
      "SQLite",
      "NextAuth.js",
      "Tailwind CSS",
      "REST APIs",
      "GitHub",
      "Vercel",
    ],
  },
  {
    title: "Business Analysis & Documentation",
    skills: [
      "Agile (Scrum, Kanban)",
      "Waterfall",
      "BRD",
      "BPMN / BPM",
      "User Stories & Acceptance Criteria",
      "SRS",
      "FRS",
      "Requirements Elicitation & Analysis",
      "Wireframing & Prototyping",
      "UAT",
      "SOPs & User Manuals",
    ],
  },
  {
    title: "Project & Delivery Management",
    skills: [
      "Sprint Planning",
      "Backlog Grooming",
      "Stakeholder Management",
      "Risk Management",
      "Resource Allocation",
      "Statements of Work",
      "Cross-Functional Collaboration",
    ],
  },
  {
    title: "Tools",
    skills: [
      "Jira",
      "Azure DevOps",
      "Asana",
      "Trello",
      "ClickUp",
      "Figma",
      "Balsamiq",
      "Lucidchart",
      "Visio",
      "Notion",
      "Microsoft Office Suite",
    ],
  },
];

export const experience = [
  {
    role: "Business Analyst & Product Owner",
    company: "Reporteq (Pvt.) Ltd",
    period: "Dec 2022 - Present",
    highlights: [
      "Delivered 50+ client projects across healthcare, real estate, automotive, and AI-driven platforms, maintaining a 95% client satisfaction rate.",
      "Authored BRDs, BPMN process models, user stories, and acceptance criteria; ran stakeholder workshops across 50+ client accounts.",
      "Designed, built, and deployed Stratum AI, a full-stack KPI tracking and AI-recommendation platform, using Claude Code to scope and ship an MVP.",
      "Rebuilt a legacy Grants Management System by feeding the existing codebase into Claude Code to redesign, refactor, and deploy a production-ready version.",
      "Led team-wide adoption of AI-assisted documentation tools, cutting first-draft documentation time by roughly 50% for BRDs, user stories, and test cases.",
      "Directed QA and UAT cycles, facilitated agile ceremonies, and managed delivery tracking via Jira and Azure DevOps across concurrent client engagements.",
    ],
  },
  {
    role: "Business Development Executive",
    company: "InvoZone (Pvt.) Ltd",
    period: "Jun 2022 - Nov 2022",
    highlights: [
      "Built and managed a CRM pipeline for prospective Web3, blockchain, and metaverse clients, translating technical concepts into clear business communication.",
    ],
  },
  {
    role: "Business Development Associate",
    company: "Programmers Force",
    period: "Oct 2021 - Dec 2021",
    highlights: [],
  },
  {
    role: "Sales and Marketing Intern",
    company: "Honda Atlas Cars (Pakistan) Limited",
    period: "Oct 2020 - Dec 2020",
    highlights: [],
  },
];

export const certifications = [
  { name: "Requirements Elicitation and Analysis", issuer: "Project Management Institute (PMI)" },
  { name: "Business Analysis for Project Managers", issuer: "Project Management Institute (PMI)" },
  { name: "Agile Project Management with Jira Cloud", issuer: "Project Management Institute (PMI)" },
  { name: "Figma: Handing Off to Developers", issuer: "LinkedIn Learning" },
];

export const education = {
  degree: "Master of Business Administration (MBA), Management & Finance",
  school: "Hailey College of Banking and Finance, University of the Punjab",
  period: "Dec 2021",
};

export const nav = [
  { label: "About", href: "#about" },
  { label: "Skills", href: "#skills" },
  { label: "Experience", href: "#experience" },
  { label: "Contact", href: "#contact" },
];
